...
public class UserRestMapper {
  ...
  public RegisterUserRequest toApplicationRequest(
      com.tickon.identityservice.user.infrastructure.adapters.dto.RegisterUserRestRequest
          webRequest) {
    return new RegisterUserRequest(
        webRequest.firstName(),
        webRequest.lastName(),
        new Username(webRequest.username()),
        new Email(webRequest.email()),
        webRequest.password());
  }

  public com.tickon.identityservice.user.infrastructure.adapters.dto.UserRestResponse toWebResponse(
      UserResponse appResponse) {
    return new com.tickon.identityservice.user.infrastructure.adapters.dto.UserRestResponse(
        appResponse.id().value().toString(),
        appResponse.firstName(),
        appResponse.lastName(),
